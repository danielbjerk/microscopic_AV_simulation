...
class Vehicle:
    def __init__(self, route, time):
        ...
        self.route = route
        # Length of all roads on route
        self.full_dist = np.sum([road.length for road in route.roads])
        self.traversed_dist = 0

        # length of vehicle i
        self.l = 4

        # Max accel of vehicle i    # 4s
        self.a_max = 2
        # comfortable deceleration of vehicle i
        self.b_max = 4.61
        # max desired speed of vehicle i. Set this as road.v_limit?
        self.v_max = 16.6
...
class DumbVehicle(Vehicle):
    def __init__(self, route, time, config={}):
        ...
        # Parameters for idm:
        # Reaction time of vehicle i's driver. Set to 0 when self.smart==True.
        self.T = 1.0
        # smoothness of the acceleration
        self.delta = 4
        # min desired distance between vehicle i and i-1.
        self.s0 = 8
    ...
    def normal_acceleration(self, car_infront):
        if car_infront:
            ...
            s_desired = self.s0+self.v*self.T+self.v * \
                (delta_v)/(2*np.sqrt(self.a_max*self.b_max))
